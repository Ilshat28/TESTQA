...
@tree
@ВидеоИнструкция
Функционал: Видео Маркировка ПРОИЗ

Сценарий: Видео Маркировка ПРОИЗ
* В этом видео научимся содавать Документы маркировки товаров
* Сперва мы должны создать документ Эмиссии, для этого нажимаем на вкладку слева ПРОИЗВОДСТВО
...
* Далее выбираем Документ Заказ на эмиссию КМ			
...
* Способ ввода в оборот выбираем Производство в РФ
...
* В поле вид продукции можем ввести либо одежду, либо обувь 
	И я показываю подсказку "Выбираем вид продукции" EnjoyHint у картинки "31" на экране
		   ...
		   | 'Толщина'      | '3'    |	
	И пауза 2
* Далее переходим на вкладку товары и начинаем добавлять нужные нам товары	, можно добавить несколько номенклатур при желании
...
* Заполняем поле характеристика, если товар разбит на характеристики	
	...
	И в таблице "ОстаткиПродукции" я перехожу к строке:
		...
		| 'Куртки утепленные JK-JP-0010-03-2122 (Лайм)' | '1'       |
